...
export function ScoutPage() {
  ...
  const handleRunScout = async () => {
    ...
    try {
      ...
      toast.error(
        `Scout failed: ${err instanceof Error ? err.message : "Unknown error"}`
      );
    } finally {
      setIsRunning(false);
    }
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Enter a make, model, and optional state. Scout will find active
            listings and build a price distribution with trend analysis.
          </CardDescription>
        </CardHeader>
                ...
                <CardContent>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-4">
                    <div className="rounded-lg bg-cyan-50 p-3 text-center">
                      ...
                      <div className="text-lg font-bold text-cyan-700">
                        ${snap.priceMedian.toLocaleString()}
                      </div>
                    </div>
                  ...
                  <div className="mb-4">
                    <div className="flex items-center justify-between text-[10px] text-muted-foreground mb-1">
                      <span>${snap.priceMin.toLocaleString()}</span>
                      <span>${snap.priceMax.toLocaleString()}</span>
                    </div>
                    <div className="h-3 bg-muted rounded-full overflow-hidden relative">
                      {(() => {
                        const range = snap.priceMax - snap.priceMin || 1;
                        const p25 =
                          ((snap.priceP25 - snap.priceMin) / range) * 100;
                        const p75 =
                          ((snap.priceP75 - snap.priceMin) / range) * 100;
                        const med =
                          ((snap.priceMedian - snap.priceMin) / range) * 100;
                        return (
                          <>
                            <div
                            ...
                            />
                            <div
                              className="absolute h-full w-0.5 bg-cyan-600"
                              style={{ left: `${med}%` }}
                            />
                          </>
                        );
